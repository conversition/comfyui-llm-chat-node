...
# 全局日志缓冲区（供外部捕获 Agent 运行日志）
_log_buf: _io.StringIO | None = None


def _append_log(plain: str) -> None:
    """向全局日志缓冲区追加一行纯文本日志。"""
    global _log_buf
    if _log_buf is not None:
        _log_buf.write(plain + "\n")


def enable_log_capture() -> None:
    """启用日志捕获，清空并创建新的 StringIO 缓冲区。"""
    global _log_buf
    _log_buf = _io.StringIO()


def get_captured_log() -> str:
    """获取已捕获的日志内容，不会关闭或清空缓冲区。"""
    global _log_buf
    if _log_buf is None:
        return ""
...
def _log(msg, color=""):
    ...
    line = plain
    if color:
        line = f"{prefix} {color}{msg}{_C.ENDC}"
...
class PromptAgent:
    def __init__(self, api_key, api_url, model_name, mode, thinking, config, effort="Medium", unique_id=None,
                 custom_prompt="", custom_temperature=0.7, custom_max_tokens=8192):
        ...
        self.custom_max_tokens = custom_max_tokens
    ...
    def _fallback_normal(self, user_text, image):
        ...
        try:
            resp = self.llm.chat.completions.create(
                ...
                temperature=0.7, max_tokens=16384, extra_body=self._extra_body,
            )
    ...
    def _force_final_output(self, messages):
        ...
        # 补充格式提醒：Agent 系统提示词不含格式规范，最后再强调一次
        # 避免模型只输出原始标签而不含 ## Prompt / ## 中文解释 结构
        fmt_hint = ""
        if self.mode == "Anima":
            fmt_hint = (
                ...
                "## 中文解释\n"
                "[分点说明设计逻辑]\n\n"
                "禁止输出思考过程，禁止使用<think>标签，直接输出最终结果。"
            )
        else:
            fmt_hint = (
                "\n\n请严格按照 XML 格式输出，包含 <img> 标签及代码块外的中文翻译。"
                "禁止输出思考过程，禁止使用<think>标签，直接输出最终结果。"
            )

        messages.append({
            "role": "user",
            "content": "请根据已收集到的标签信息直接输出最终 prompt，禁止再调用任何工具。" + fmt_hint,
        })
        try:
            resp = self.llm.chat.completions.create(
                model=self.model_name,
                messages=_sanitize_messages_for_gemini(messages),
                temperature=0.7, max_tokens=self.custom_max_tokens, extra_body=self._extra_body,
            )
            content = resp.choices[0].message.content or ""
            # 剥离思考标签：优先匹配闭合标签，无闭合标签时从 <think> 截到末尾
    ...
    def _retry_short_output(self, messages, prev_content):
        """输出过短时重试：追加"重新生成"指令，单次非工具调用。

        返回 (new_content, success)，success=False 时使用旧 content 兜底。
        """
        _log_warn(f"输出过短（{len(prev_content)} chars），尝试重新生成...")
        retry_msg = {
            "role": "user",
            "content": (
                "你上一步的输出内容不完整或格式不正确，请重新生成。\n\n"
                "要求：严格按照规范格式输出完整内容，包含 ## Prompt 和 ## 中文解释 标题，"
                "不要输出思考过程，不要使用 <think> 标签，直接输出最终结果。"
            ),
        }
        try:
            resp = self.llm.chat.completions.create(
                model=self.model_name,
                messages=_sanitize_messages_for_gemini(messages + [retry_msg]),
                temperature=0.7, max_tokens=16384, extra_body=self._extra_body,
            )
            new_content = resp.choices[0].message.content or ""
            if re.search(r'<think>', new_content):
                new_content = re.sub(r'<think>.*?</think>', "", new_content, flags=re.DOTALL)
                new_content = re.sub(r'<think>.*', "", new_content, flags=re.DOTALL)
            new_content = new_content.strip()
            if len(new_content) > len(prev_content):
                return new_content, True
        except Exception as e:
            _log_error(f"重试 LLM 调用失败: {e}")
        return prev_content, False
    ...
    def run(self, user_text, image=None):
        ...
        # 输出过短保险：xml_out 不足 25 个有效字符 → 重试一次
        if len(xml_out.strip()) < 25:
            _log_warn(f"xml_out 过短（{len(xml_out.strip())} chars），触发重试保险")
            retry_content, ok = self._retry_short_output(messages, content)
            if ok:
    ...
    def _build_cold_run(self, user_text, image):
        ...
        # 追加用户自定义指令（UI 的 system_prompt 输入）
        ...
        while rounds < max_rounds:
            ...
            try:
                resp = self.llm.chat.completions.create(
                    ...
                    tool_choice="auto", temperature=0.7, max_tokens=16384,
                    extra_body=self._extra_body,
                )
            except Exception as e:
